...
router.get('/', async (req, res) => {
  ...
  if (user_id?.length > 0 && company_id?.length > 0 && scope?.length > 0) {
    ...
    res.sendStatus(401);
    log.info('Unauthorized', JSON.stringify({ scope, custom_id, user_id, company_id, headers: req.headers }));
  }
});
...
router.post('/login', async (req, res) => {
  ...
  try {
    ...
    await email.sendAuth(user.email, `https://${req.body.type === 'admin' ? admin_front : front}/auth/authenticate/?${qs.stringify(payload)}`, user.code);
  } catch (e) {
    console.log(e);
    log.warning(e);
  }
...
router.post('/authenticate', async (req, res) => {
  ...
  try {
    ...
    if (user.code === code && updated_at > new Date()) {
      ...
      log.info('Bearer for', { user, tokens });
